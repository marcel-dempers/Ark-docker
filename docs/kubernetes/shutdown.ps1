...
while ($delayMinutes -ge 0)
{
  ...
  {
    kubectl -n ark exec -it ark-server-0 -- arkmanager broadcast "Server will stop in $delayMinutes min. Reason: $ENV:REASON"
  }
...
kubectl -n ark exec -it ark-server-0 -- arkmanager broadcast "Server shutting down... Reason: $ENV:REASON"
